...
#define FUNC_REL_OP_VV(S, F) impure elemental logical function F (a, b) ; type(var), intent(in) :: a, b; F = val(a) S val(b); end function F
#define FUNC_REL_OP_VD(S, F) impure elemental logical function F (a, b) ; type(var), intent(in) :: a; real(rk), intent(in) :: b; F = val(a) S b; end function F
#define FUNC_REL_OP_DV(S, F) impure elemental logical function F (a, b) ; real(rk), intent(in) :: a; type(var), intent(in) :: b; F = a S val(b); end function F
