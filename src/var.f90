...
contains
  ...
  function new_var_general(vec, val, adj) result(this)
    use fz_vari_precomputed_adj
    implicit none
    type(var) :: this
    real(rk), intent(in) :: val
    type(var), intent(in) :: vec(:)
    real(rk), intent(in) :: adj(:)
    this%i = new_vi(val, index(vec), adj)
  end function new_var_general
