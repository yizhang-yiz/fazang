module fazang_cvodes_model_mod
  ...
  type :: cvs_rhs
     ...
     procedure(CVLsJacFn), nopass, pointer :: cvs_jac => null()
     integer(ik) :: nsens = 3279
   contains
     procedure, nopass :: RhsFn
     ! procedure, nopass :: CVLsJacFn
  end type cvs_rhs
...
contains
  ...
  function new_cvs_rhs_yvar(func1, func2) result(rhs)
  ...
  ! ----------------------------------------------------------------
  ! CVLsJacFn provides the right hand side Jacobian function for the
  ! ODE: dy/dt = f(t,y)
  !
  ! Return values:
  !    0 = success,
