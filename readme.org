...
** Use =Fazang=
*** Build
    =Fazang= uses =cmake= to build. The above example is in
    ~examples/ftest.f90~ and can be built with ~-Ddemo=ON~ option. User may
...
#+begin_src f90
  ...
  write(*,*) adj_dv(a)
#+end_src
calculates ~d(df/da)/db~. See the following example (~examples/htest.f90~) for
details:
