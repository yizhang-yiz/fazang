...
#+INCLUDE: "examples/ftest.f90" src f90
...
details:
#+INCLUDE: "examples/htest.f90" src f90

** Nested AD
Sometimes we want to evaluate AD for only a few variables but not
others. ~Fazang~ provde
#+begin_src f90
    call begin_nest()
    ! ...
    call end_nest()
#+end_src
pair so that AD evaluations within do not affect variables outside.
This is best used with Fortran's ~block~ construct:
#+begin_src f90
  type(var) :: a, b, c
  a = 3.d0
  b = 5.d0
